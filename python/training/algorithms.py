# ===============================================================================
# This script is used for returning either a SAC or PPO Config using RLlib

# Manipulation.py is called to register an environment so that the configuration
# of the environment is returned algorithm along with training, resources 
...
def load_yaml_config() -> dict:
    ...
    cfg["training"]["experiment_name"] = cfg["training"]["experiment_name"].replace("${algorith}", cfg["algorithm"])
    return cfg
...
def _apply_common(config, cfg:dict):
    ...
    return (
        ...
        .env_runners(
            ...
            num_cpus_per_env_runner = res_cfg["num_cpus_per_env_runner"],
        )
        .learners(
            num_gpus_per_learner    = res_cfg["num_gpus"]
        )
    )
...
def build_algo_config(algorithm: str | None = None, overrides: dict | None = None) -> PPOConfig:
    ...
    'algorithm' defaults to config.yaml's top-level algorithm field
    'overrides' lets an Optuna study replace hyperparameters for whichever algorithm is selected
